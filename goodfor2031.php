<?
  ...
  date_default_timezone_set("Asia/Hong_Kong");
    ...
    $sql = "CREATE TABLE goodfor2031
    (
    item varchar(255),
    date varchar(255)
    )";
    $delsql = "DROP table goodfor2031";
    ...
    function inputdata($item) {
      ...
      $conn = mysql_connect($servername, $username, $password, $dbname);
      mysql_select_db($dbname, $conn);
      $sql1 = "insert into goodfor2031(item,date) values('".$item."','".date("Y-m-d")."')";
  ...
  for($bi = 1; $bi < 40; $bi++)
  {
    if($bi == 1)
    {
      $url = "https://www.ebay.com/sch/m.html?_ssn=goodfor2031&_sop=10&_armrs=1&_from=R40&_sacat=0&_nkw=human+hair+wig&rt=nc&_ipg=200";
  //  $url = "https://www.ebay.co.uk/sch/m.html?_fcid=77&_ssn=".$_POST[$SWITCHS]."&_armrs=1&_clu=2&gbr=1&_from=R40&_sacat=0&_nkw=".$_POST['sch']."&rt=nc&_fcid=3";
      ...
      $dv = $bi - 1 ;
      $skc = $dv * 2 * 100;
     $url = "https://www.ebay.com/sch/m.html?_ssn=goodfor2031&_sop=10&_armrs=1&_from=R40&_ipg=200&_sacat=0&_nkw=human+hair+wig&_pgn=".$bi."&_skc=".$skc."&rt=nc";
