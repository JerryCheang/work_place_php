...
include('ss/conn.php');
$stmt = $db->query('select * from aliexpress_item'); //返回一个PDOStatement对象
...
for( $i= 0; $i < $row_count; $i++ ){
  ...
  $path = 'aliexpress_feedback_images/'.$rows[$i]["keyword"];
  $i_page = 1;
  ...
  while(1){
    ...
    $i_page++;
    $matches = null;
    $aliexpress_feedback_list_url = $rows[$i]["url"]."&withPictures=true&page=".$i_page;
    ...
    for( $i=0; $i < count($matches[0]); $i++ ){
      ...
      echo $matches[0][$i]."\n";
      download( $matches[0][$i] , $path );

        }

    }

  $sql_write = "update aliexpress_item set status='OK' where "."keyword='".$rows[$i]["keyword"]."'"." and url='".$rows[$i]["url"]."'" ;
  $db->query($sql_write);
}
