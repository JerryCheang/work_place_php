function cnVeryCalendar() {
	...
	this.generateCalendarTable = function() {
		...
		/* Frame Table Header */
		body += "<table align='center' cellpadding='1' cellspacing='1' width='100%' height='100%' style='border:1px "
				+ border_frame + " solid; background:white;'>";
		body += " <tr>";
		body += "   <td style='background:" + back_frameCaption + ";"
				+ font_cnChar + "' height='20'>";
		body += "    <div style='color:" + fore_frameCaption
				+ "; float:left'>&nbsp;选择出发日期</div>";
		body += "    <div style='float:right'>";
		body += "     <a href=\"javascript:calendar.setValue('')\" style='color:"
				+ fore_frameCaption
				+ "; text-decoration:none;"
				+ font_cnChar
				+ "'>[擦除]</a> ";
		body += "     <a href='javascript:calendar.fadeOut()' style='color:"
				+ fore_frameCaption + "; text-decoration:none;" + font_cnChar
				+ "'>[关闭]</a>";
		body += "    </div>";
		body += "   </td>";
		body += " </tr>";
		...
		body += "      <td align='center' nowrap='nowrap' style='color:"
				+ fore_currentMonth + ";" + font_cnChar + "'><b>" + y + "年" + m
				+ "月</b></td>";
		...
		body += "<table align='center' width='99%' cellpadding='3' cellspacing='0' border='1' bordercolor='"
				+ border_inner
		...
		if (j != 0) {
			...
			body += ("<td style='background:" + back_nullDay + style_cell
					+ "' colspan='" + j + "'></td>");
		}
		...
		for (i = 1; i <= k; i++) {
			...
			var nowYearMonthDate = today.getFullYear() * 10000
					+ (today.getMonth() + 1) * 100 + today.getDate();
			var numYearMonth = y * 100 + m;
			var nowYearMonth = today.getFullYear() * 100
					+ (today.getMonth() + 1);
			if (numYearMonthDate >= nowYearMonthDate - 1) {
				body += " onmouseover=\"this.style.backgroundColor='"
						+ back_dayMouseOver + "'; this.style.color='"
						+ fore_dayMouseOver + "'\"";
				...
				body += " style='" + font_numChar + style_gray + "'";
			}
		...
		if (j < (36 - k)) {
			body += ("<tr><td colspan='7' style='background:" + back_nullDay
					+ style_cell + "'>&nbsp;</td></tr>");
		}
		if (j == 0 && k == 28) {
			body += ("<tr><td colspan='7' style='background:" + back_nullDay
	...
	this.loadPreviousYear = function() {
		y--;
		document.getElementById("__cnVeryCalendarContainer").innerHTML = this
	...
	this.setHook = function(dateField) {
		if (document.getElementById("__cnVeryCalendarContainer").style.display != 'none'
				&& reciever.id == dateField.id) {
		...
		/*
		 * document.getElementById("__cnVeryCalendarContainer").style.left =
		 * (point.x + dateField.offsetWidth + 5) + 'px';
		 */
		/*
		 * document.getElementById("__cnVeryCalendarContainer").style.top =
		 * point.y + 'px';
		 */
...
var hhttmmll="<div id='__cnVeryCalendarContainer' style='width:200px; height:190px; position:absolute; float:left; display:none; z-index:999'></div>";
/*Render Instance*/
