...
$sql = "CREATE TABLE "."MKHE_".date("Ymd")."
(
...
$delsql = "DROP table "."MKHE_".date("Ymd");
...
function inputdata($item) {
  ...
  $conn = mysql_connect($servername, $username, $password, $dbname);
  mysql_select_db($dbname, $conn);
  $sql1 = "insert into "."MKHE_".date("Ymd")."(sellerid,img,ititle,itemid) values('".$sellerid[1]."','".$img[1]."','".$ititle[1]."','".$itemid[1]."')";
...
$sch = "";
  if( $i == 0 )
  {
 echo  $anay = vget("https://www.ebay.com/sch/i.html?_sacat=0&_nkw=human+hair+extensions&rt=nc&_ipg=200");
  }else{
    $skc = $i * 200;
    $png = $i + 1;
 echo  $anay = vget("https://www.ebay.com/sch/i.html?_sacat=0&_nkw=human+hair+extensions&_pgn=".$png."&_skc=".$skc."&rt=nc");
  }
