...
//登录后要获取信息的地址
$url2 = $work_website."/index.php/muban/editmuban/muban_id/".$_POST['mubanid'];
//模拟登录
...
/* 解析cookie结束 */
  ...
  $rs = str_replace("</html>","", $rs);
  $rs = str_replace("/index.php",$work_website."/index.php", $rs);
  $rs = str_replace("/link",$work_website."/link", $rs);
  ...
  $rs = str_replace("<div style=\"float:left\">","<input id=\"paypal\" name=\"paypal\" value=\"", $rs);
	...
	$rs = str_replace('<form action="" method="post" name="m" id=\'m\' target="_self">',"<form action=\$work_website."/index.php/muban/editmuban/muban_id/\" method=\"post\" name=\"m\" id=\"m\" target=\"_self\">", $rs);
...
}else if($_POST["submit"] == "Edit Muban")
{
	...
	<body>
	<script language='javascript'>document.location = '".$work_website."index.php/muban/editmuban/muban_id/".$_POST['mubanid']."'</script>
	</body>
	</html>";
}
?>
