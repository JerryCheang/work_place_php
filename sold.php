...
//登录后要获取信息的地址
$url2 = $work_website."/index.php?controller=dingzhi&action=transactionsoldv2";
...
while($row = mysql_fetch_array($result))//转成数组，且返回第一条数据,当不是一个对象时候退出
{
 $site[$i] = $row['site'];
 $sku[$i] = $row['sku'];
 $selleruserid[$i] = $row['selleruserid'];
 $i++;
}
$sql = "CREATE TABLE "."SOLD_".date("Ymd",strtotime($XXA." day"))."
(
site varchar(255),
sku varchar(255),
...
$delsql = "DROP table "."SOLD_".date("Ymd",strtotime($XXA." day"));
